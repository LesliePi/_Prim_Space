# -*- coding: utf-8 -*-
"""
Created on Tue Apr 21 22:03:24 2026
...
#!/usr/bin/env python3
"""
Primspace Core Engine v3.1 - NO LATEX in labels
...
# Disable LaTeX in matplotlib completely
...
class ResidualDensity:
    """alpha_k(n) = (1/k) * sum (n mod p_i)/(p_i - 1)"""
...
def create_multi_layer_visualization(
    ...
    """Four-panel Primspace visualization - NO LATEX"""
    ...
    ax1.set_xlabel(f'n mod {moduli[0]}')  # NO LATEX
    ax1.set_ylabel(f'n mod {moduli[1]}')  # NO LATEX
    ...
    ax2.axhline(y=0.6, color='green', linestyle='--', alpha=0.5, label='Threshold alpha = 0.6')
    ax2.set_title(f'Layer 2: Residual Density alpha_k(n)\nk={len(moduli)}', fontsize=12, fontweight='bold')
    ax2.set_xlabel('n')
    ax2.set_ylabel('alpha_k(n)')
    ...
    ax4.set_title(f'Distribution of alpha_k(n)\nPrimes vs Composites', fontsize=12, fontweight='bold')
    ax4.set_xlabel('alpha_k(n)')
    ax4.set_ylabel('Density')
    ax4.legend(loc='upper left', fontsize=9)
    ax4.grid(True, alpha=0.3)
    
    plt.suptitle(f'Primspace Unified Framework | n = 1 to {n_max} | {len(prime_db.primes):,} primes loaded',
    ...
    stats_text = f"Primes with alpha_k > 0.6: {prime_ratio_high:.1%} | Composites with alpha_k > 0.6: {comp_ratio_high:.1%}"
...
def analyze_prime_distribution(prime_db: PrimeDatabase = None, n_max: int = 100000):
    ...
    print(f"  Actual pi({n_max}): {n_primes:,}")
...
def create_twin_prime_visualization(
    ...
    """Enhanced twin prime visualization - NO LATEX"""
    ...
    for idx, (p1, p2) in enumerate(twin_pairs[:n_plots]):
        ...
        ax.set_title(f'Twin Prime Pair ({p1}, {p2}) | alpha_k drop: {drop:.3f}', fontsize=11)
        ax.set_xlabel('n')
        ax.set_ylabel('alpha_k(n)')
        ax.set_ylim(0, 1)
...
if __name__ == "__main__":
    print("=" * 70)
    print("PRIMSPACE CORE ENGINE v3.1")
    print("Using prime database from CSV (NO LATEX)")
    print("=" * 70)
